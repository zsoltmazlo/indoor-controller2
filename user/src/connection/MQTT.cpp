...
MQTT::MQTT(char* domain, uint16_t port, callback_t cb) {
    this->initialize(domain, NULL, port, MQTT_DEFAULT_KEEPALIVE, cb, MQTT_MAX_PACKET_SIZE);
}

MQTT::MQTT(char* domain, uint16_t port, callback_t cb, int maxpacketsize) {
    this->initialize(domain, NULL, port, MQTT_DEFAULT_KEEPALIVE, cb, maxpacketsize);
}

MQTT::MQTT(uint8_t *ip, uint16_t port, callback_t cb) {
    this->initialize(NULL, ip, port, MQTT_DEFAULT_KEEPALIVE, cb, MQTT_MAX_PACKET_SIZE);
}

MQTT::MQTT(uint8_t *ip, uint16_t port, callback_t cb, int maxpacketsize) {
    this->initialize(NULL, ip, port, MQTT_DEFAULT_KEEPALIVE, cb, maxpacketsize);
}

MQTT::MQTT(char* domain, uint16_t port, int keepalive, callback_t cb) {
    this->initialize(domain, NULL, port, keepalive, cb, MQTT_MAX_PACKET_SIZE);
}

MQTT::MQTT(char* domain, uint16_t port, int keepalive, callback_t cb, int maxpacketsize) {
    this->initialize(domain, NULL, port, keepalive, cb, maxpacketsize);
}

MQTT::MQTT(uint8_t *ip, uint16_t port, int keepalive, callback_t cb) {
    this->initialize(NULL, ip, port, keepalive, cb, MQTT_MAX_PACKET_SIZE);
}

MQTT::MQTT(uint8_t *ip, uint16_t port, int keepalive, callback_t cb, int maxpacketsize) {
    this->initialize(NULL, ip, port, keepalive, cb, maxpacketsize);
}
...
void MQTT::initialize(char* domain, uint8_t *ip, uint16_t port, int keepalive, callback_t cb, int maxpacketsize) {
    this->callback = cb;
                ...
                if (type == MQTTPUBLISH) {
                    if (callback) {
                        ...
                        if ((buffer[0]&0x06) == MQTTQOS1_HEADER_MASK) {
                            ...
                            callback.operator ()(topic,payload,len-llen-3-tl-2);
...
//                            callback(topic,payload,len-llen-3-tl);
                            callback.operator ()(topic,payload,len-llen-3-tl);
                        }
                    } else {
                        debug_print("No callback declared");
                    }
